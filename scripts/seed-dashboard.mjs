...
const main = async () => {
  ...
  await prisma.weeklyReview.upsert({
    ...
    update: {
      ...
      mainEnergyCost: '主要内耗来自信息切换、碎片消息和晚上注意力被分散。',
      repeatedPatterns: '当任务被拆成明确的小块时，推进感更强；当输入过多时，内耗会上升。',
      highFrequencyTags: JSON.stringify(['真实建设感', 'AI 开发', '掌控感', '恢复节奏', '低压力推进']),
      nextWeekAction: '为重要项目设定一个「单点突破」目标，聚焦最能产生复利的那一件事。',
      aiSummary:
        '你本周在「职业推进」上投入稳定，且能兼顾关系连接与自我恢复，整体节奏较为平衡。内耗主要来自信息处理与多线程切换，建议为深度工作预留更不被打扰的时间段。',
  ...
  await prisma.aiAnalysis.create({
    data: {
      ...
      behaviorPatterns: '低压力推进、碎片化内耗、明确小块任务带来掌控感',
      constructivenessNote: '建设感主要来自可展示项目、组件拆分和清晰的下一步。',
      energyCostNote: '内耗主要来自消息切换、资讯输入和夜间注意力分散。',
